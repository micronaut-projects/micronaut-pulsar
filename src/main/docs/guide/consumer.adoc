...
snippet::example.ConsumerProducer[project-base="doc-examples/example", source="main", indent="0"]

[.lang-python]
****
WARNING: The Python compiler (`micronaut-inject-python`) does not support `@PulsarConsumer` methods yet: it treats a method decorated with `@PulsarConsumer` as a `@Bean` factory method and rejects it, so the decorator is commented out in the Python example. Producers, producer clients and readers work as shown.
****

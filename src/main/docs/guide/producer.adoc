...
snippet::example.Producer[project-base="doc-examples/example", source="main", indent="0"]
...
[.lang-python]
****
In Python the producer client is an abstract class (`ABC`) whose `@PulsarProducer` methods are `@abstractmethod`s with `...` bodies.
****
